...
| [OPERATE.md](OPERATE.md) | One ordered operator list (ROBUST-01) | clone → hub preview → validate → optional graph → Crosscheck write-through; skip conditions; no launch steps |
| [CATALOG_BATCH.md](CATALOG_BATCH.md) | One local catalog batch (WORK-02) | YAML add → validate → graph → dashboard consistency → one reviewed change. Wave Factory stays in [PATH_TO_SUCCESS.md](PATH_TO_SUCCESS.md) |
...
| [PATH_TO_SUCCESS.md](PATH_TO_SUCCESS.md) | **Ops appendix** (catalog wave checklist). Product path is repo-root [ROADMAP.md](../ROADMAP.md) | Do not treat May 2026 Reddit/domain/arXiv-this-week list as current |
| [CREW.md](CREW.md) | Night crew + backlog scout; machines propose, humans promote | `scripts/run_crew.py`; `scripts/backlog_scout.py`; harvest + `backlog-scout.yml`; `drafts/crew-reports/` |
| [CROSSCHECK.md](CROSSCHECK.md) | RESULT write-through | `scripts/apply_crosscheck_result.py`; hub last-run badge via `render_crosscheck_hub.py` |
| [ROADMAP.md](../ROADMAP.md) | **Canonical product path** (now / shipped / parked / later) | GSD execution list is [`.planning/ROADMAP.md`](../.planning/ROADMAP.md) |
