...
|-------------|--------------|------------|-------|-------------------|-----------------|
| `p-b-habitat-percolation-ecology-fss` | `simulate_percolation_fss.py` — frozen estimator **mean-first-either-wrap** (2D site percolation, periodic 4-neighbor square lattice, Newman–Ziff; \(p_c(L)\) is the mean occupation fraction at first wrapping in either direction). Fit L∈{32,64,128,256}, `N_SAMPLES=400`, L=16 diagnostic. SE = sample SE of first-wrap occupation, not binomial open-Π SE. CI also runs `--seed 123`. | yes, smoke only (`L∈{16,32,48,64}`, 48 samples; **must not** emit CONFIRMED and does not recover \(\nu\)) | no | yes (seed 42 + seed 123) | stdout `RESULT:` token (`CONFIRMED` / `INCONCLUSIVE` / `FALSIFIED`); Python `return 0` always. `INCONCLUSIVE` = underpowered, not “percolation is wrong.” |
