...
- **`docs/CREW.md`:** roles, mailbox, what the crew must not do.
- **Shipper:** `scripts/crew_ship.py` squash-merges the bot PR only if files are harvest JSON + `drafts/crew-reports/`. Catalog / repro / schemas are refused. Branch protection may still leave the PR open.
