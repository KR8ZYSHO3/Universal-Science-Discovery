...
### Added — Crosscheck RESULT write-through (WORK-01)
- **`scripts/apply_crosscheck_result.py`:** records stdout `RESULT:` onto protocol YAML as `last_run_result` / `last_run_at`. Does not set `status: confirmed`.
- Hub Crosscheck cards show a **last run** badge (including honest INCONCLUSIVE). Refresh with `render_crosscheck_hub.py --apply`.
...
- Merge-check freeze: estimator id `mean-first-either-wrap` (either-direction first wrap, not one-axis); SE is sample SE of first-wrap occupation, not binomial open-Π; \(L=32\) kept with an explicit corrections-to-scaling note; CI second seed 123; ecology sentence is finite-landscape shift, not a torus park-design constant.
...
- **`dashboard/index.html`:** **`deploy-info`** empty-SHA banners distinguish **GitHub Pages vs local**; **`bridge_claim` / `systematic_gaps`** use **`String(...)`** before **`.substring`**; activity feed guards missing **`commit.message`**; **bridge proposals** fetch tolerates **non-OK** HTTP.
