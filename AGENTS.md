...
- **Product path:** repo-root **`ROADMAP.md`** is the only strategy. **`docs/USE.md`** is the only utilization flow (Look / Add / Run). **`docs/PATH_TO_SUCCESS.md`** is a catalog-wave checklist. **`LAUNCH_PLAYBOOK.md`** / launch sprint docs are parked. Do not market, post, or submit arXiv unless the owner un-parks launch.
- Do not add a new “start here” guide. Point at **`docs/USE.md`**. Door 2 YAML detail stays **`docs/HAPPY_PATH_FIRST_RECORDS.md`**.
