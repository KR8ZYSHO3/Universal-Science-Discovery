...
- **Product path:** repo-root **`ROADMAP.md`** is the only strategy. **`docs/USE.md`** is the only utilization flow (try it / add a question / keep it honest). **`docs/PATH_TO_SUCCESS.md`** is a catalog-wave checklist. **`LAUNCH_PLAYBOOK.md`** / launch sprint docs are parked. Do not market, post, or submit arXiv unless the owner un-parks launch.
- Front-door copy uses researcher language (record, send for review, operate the site). GitHub words (YAML, PR, clone, CI) belong in the contributor guide, not the hub hero.
- Do not add a new “start here” guide. Point at **`docs/USE.md`**. First-records detail stays **`docs/HAPPY_PATH_FIRST_RECORDS.md`**.
