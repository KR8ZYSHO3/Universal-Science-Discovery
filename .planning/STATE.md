...
**Recent Launch Ship (this update):** Pushed core launch artifacts + dashboard institutional polish to PR #267 (branch launch/june-2026-shipping). Includes INSTITUTIONAL_PARTNERSHIP_PROSPECTUS.md, LAUNCH_* planning files, QUICK_START_LAUNCH_SPRINT.md, early_stewards_call.md, full hero rewrite with Launch Sprint banner + researcher/institutional CTAs and value block, mkdocs nav updates (incl. MILESTONES.md, launch_posting_calendar.md, cover_letter.md, Energy Harvesting Provocation page), link checker config fixes. Dashboard stats synced via `update_dashboard_stats.py --apply` (fixed test_verify_dashboard_consistency: bridges 1124, phenom 11, etc. after adding p-energy-anomaly-001 etc.). All local validations (schemas, smoke 5/5, mkdocs strict) now pass. CI hygiene PR #274 (mkdocs nav) ready to merge. Major step for institutional visibility and launch CI health.
