...
- **Orphan / xref hub export:** `python scripts/export_orphan_xref_panel.py` — writes **`api/v1/orphan_xref_panel.json`** for the contributor hub panel (runs in **`build-graph.yml`** after `build_graph.py`).
...
- **Graph pathfinder (DISC-01):** `python scripts/graph_pathfinder.py --from statistical-physics --to conservation-biology` — shortest paths between domains (or `--from-id` / `--to-id`) on `docs/knowledge_graph.json`; hub UI in knowledge-graph section. Spec **`.planning/specs/PATHFINDER.md`**.
- **Impact router (DISC-02):** `python scripts/propose_bridges.py` — ranks missing bridges (pioneer seeds, breakthrough-gap blockers, domain pairs); writes `docs/bridge_proposals.json` and `api/v1/bridge_proposals.json`. Hub **`#impact-router`**.
- **Priority unknowns (DISC-03):** `python scripts/export_orphan_unknowns_panel.py` — orphan unknowns with no graph edges; writes **`api/v1/orphan_unknowns_panel.json`**. Hub **`#orphan-unknowns-panel`** (runs in **`build-graph.yml`** after `find_orphan_unknowns.py`).
